...
class Trainer:
    def init(self, opt_path, opt, launcher):
        ...
        if (
            ...
            shutil.copy(
                ...
                os.path.join(
                    opt["path"]["experiments_root"],
                    f'{datetime.now().strftime("%d%m%Y_%H%M%S")}_{os.path.basename(opt_path)}',
                ),
            )
        ...
        if opt["wandb"] and self.rank <= 0:
            ...
            wandb.init(
                project=project_name, dir=opt["path"]["log"], config=opt, name=run_name
            )
        ...
        for phase, dataset_opt in opt["datasets"].items():
            if phase == "train":
                ...
                if self.rank <= 0:
                    self.logger.info(
                        "Number of training data elements: {:,d}, iters: {:,d}".format(
        ...
        if "eval" in opt.keys() and "evaluators" in opt["eval"].keys():
            ...
            for ev_key, ev_opt in opt["eval"]["evaluators"].items():
                self.evaluators.append(
                    create_evaluator(
                        self.model.networks[ev_opt["for"]], ev_opt, self.model.env
                    )
                )
        ...
        if resume_state:
            ...
            self.total_training_data_encountered = (
                0
                if "training_data_encountered" not in opt.keys()
        ...
        if "force_start_step" in opt.keys():
            ...
            self.total_training_data_encountered = (
                self.current_step * opt["datasets"]["train"]["batch_size"]
            )
        ...
        if "val_freq" in opt["train"].keys():
            self.val_freq = (
                opt["train"]["val_freq"] * opt["datasets"]["train"]["batch_size"]
            )
        else:
    ...
    def do_step(self, train_data):
        ...
        opt = self.opt
        batch_size = self.opt["datasets"]["train"][
            "batch_size"
            ...
            if self.rank <= 0:
                ...
                if (
                    ...
                    number_of_states_to_save = (
                        opt["upgrades"]["number_of_states_to_save"]
                ...
                if opt["logger"]["disable_state_saving"] is False:
                    ...
                    self.logger.info(
                        "State saving is disabled. Skipping state save, you won't be able to resume training from this session."
                    )
        ...
        if opt_get(opt, ["eval", "pure"], False) and do_eval:
            ...
            for val_data in tqdm(self.val_loader):
                self.model.feed_data(
                    val_data, self.current_step, perform_micro_batching=False
                )
            ...
            if self.rank <= 0:
                ...
                if opt["wandb"]:
                    ...
                    wandb.log(
                        {
                            f"eval_{k}": torch.stack(v).mean().item()
                            for k, v in reduced_metrics.items()
                        }
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="../experiments/train_gpt.yml",
    )
    parser.add_argument(
